...
def generate_report(mbti_type, analysis, format="html"):
    ...
    <body>
        ...
        <div class="section">
            ...
            <div class="analysis">
                <h3>Traditional MBTI Dimensions</h3>
                <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 20px;">
                    {f"""
                    <div style="flex: 1; min-width: 150px; background: #e8f5e9; padding: 10px; border-radius: 8px; border-left: 4px solid #4CAF50;">
                        <strong>Extraversion / Introversion</strong><br>
                        {('E' if analysis.get('traditional_scores', {}).get('E_score', 0.5) > analysis.get('traditional_scores', {}).get('I_score', 0.5) else 'I')} 
                        ({max(analysis.get('traditional_scores', {}).get('E_score', 0.5), analysis.get('traditional_scores', {}).get('I_score', 0.5)) * 100:.1f}%)
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #e8f5e9; padding: 10px; border-radius: 8px; border-left: 4px solid #4CAF50;">
                        <strong>Sensing / Intuition</strong><br>
                        {('S' if analysis.get('traditional_scores', {}).get('S_score', 0.5) > analysis.get('traditional_scores', {}).get('N_score', 0.5) else 'N')}
                        ({max(analysis.get('traditional_scores', {}).get('S_score', 0.5), analysis.get('traditional_scores', {}).get('N_score', 0.5)) * 100:.1f}%)
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #e8f5e9; padding: 10px; border-radius: 8px; border-left: 4px solid #4CAF50;">
                        <strong>Thinking / Feeling</strong><br>
                        {('T' if analysis.get('traditional_scores', {}).get('T_score', 0.5) > analysis.get('traditional_scores', {}).get('F_score', 0.5) else 'F')}
                        ({max(analysis.get('traditional_scores', {}).get('T_score', 0.5), analysis.get('traditional_scores', {}).get('F_score', 0.5)) * 100:.1f}%)
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #e8f5e9; padding: 10px; border-radius: 8px; border-left: 4px solid #4CAF50;">
                        <strong>Judging / Perceiving</strong><br>
                        {('J' if analysis.get('traditional_scores', {}).get('J_score', 0.5) > analysis.get('traditional_scores', {}).get('P_score', 0.5) else 'P')}
                        ({max(analysis.get('traditional_scores', {}).get('J_score', 0.5), analysis.get('traditional_scores', {}).get('P_score', 0.5)) * 100:.1f}%)
                    </div>
                    """ if analysis.get('traditional_scores') else "<p>Traditional scores not available.</p>"}
                </div>

                <h3>Confidence Scores</h3>
                <div style="display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 20px;">
                    {f"""
                    <div style="flex: 1; min-width: 150px; background: #fff3e0; padding: 10px; border-radius: 8px; border-left: 4px solid #ff9800;">
                        <strong>E-I Confidence</strong>: {analysis.get('confidence_scores', {}).get('EI_confidence', 0):.2f}
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #fff3e0; padding: 10px; border-radius: 8px; border-left: 4px solid #ff9800;">
                        <strong>S-N Confidence</strong>: {analysis.get('confidence_scores', {}).get('SN_confidence', 0):.2f}
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #fff3e0; padding: 10px; border-radius: 8px; border-left: 4px solid #ff9800;">
                        <strong>T-F Confidence</strong>: {analysis.get('confidence_scores', {}).get('TF_confidence', 0):.2f}
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #fff3e0; padding: 10px; border-radius: 8px; border-left: 4px solid #ff9800;">
                        <strong>J-P Confidence</strong>: {analysis.get('confidence_scores', {}).get('JP_confidence', 0):.2f}
                    </div>
                    """ if analysis.get('confidence_scores') else "<p>Confidence scores not available.</p>"}
                </div>
                
                <h3>AI Insights</h3>
                <div class="analysis-content">
                    {markdown_to_html(analysis.get('llm_analysis', 'AI analysis not performed. Use the --llm flag to enable detailed personality insights.'))}
                </div>
            </div>
        </div>
